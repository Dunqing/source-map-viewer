...
Shared URLs use short IDs stored in Cloudflare KV (e.g., `/{shortId}`). The share flow: `POST /api/share` stores `{ generatedCode, sourceMapJson }` in KV with a 30-day TTL (renewed on each access), returns an 8-char ID. Inline compressed hashes (`/{base64hash}`) are supported as fallback. UI state (active tab, panel toggles) is in query params.

**LLM-friendly endpoint**: Middleware (`middleware/01.ai-markdown.ts`) intercepts `/{slug}` requests. AI bots (detected via User-Agent patterns or `Accept: text/markdown`) get a structured markdown debug report generated by `core/prompt.ts` with `core/parser-lite.ts` (a lightweight VLQ parser that works in Cloudflare Workers where `source-map-js`'s `new Function()` is blocked). Browsers get the normal SSR page via `next()`.

**Flash prevention**: When a user opens a shared URL, a blocking `<script>` in `void.json` adds `has-hash` class to `<html>` before paint. CSS rule `html.has-hash #app { visibility: hidden }` hides the SSR-rendered page. The `resolved` ref in `pages/index.vue` gates rendering — nothing shows until the slug is resolved. After resolution, `classList.remove("has-hash")` reveals the page.

### SSR CSS Injection (Dev Only)

The `unocssDevSSRInject()` plugin in `vite.config.ts` solves UnoCSS not being included in Vite's dev SSR HTML. It intercepts Cloudflare's streamed SSR response (`writeHead` → `write` chunks → `end`), buffers HTML responses, generates UnoCSS styles via the `unocss:api` plugin context, and injects them as a `<style>` tag before `</head>`. Also injects the Tailwind reset CSS and the `has-hash` hiding rule. Production extracts CSS to files during build.
